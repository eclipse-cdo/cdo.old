...
class ProtocolParserRegistryReader extends RegistryReader
{
  ...
  protected boolean readElement(IConfigurationElement element, boolean add)
  {
    if (element.getName().equals(TAG_PARSER))
    {
      ...
      {
        Object previous = Resource.Factory.Registry.INSTANCE.getProtocolToFactoryMap().put(protocolName,
        ...
        if (previous instanceof ResourceFactoryDescriptor)
        {
          ...
          EcorePlugin.INSTANCE.log("Both '" + descriptor.element.getContributor().getName() + "' and '"
              + element.getContributor().getName() + "' register a protocol parser for '" + protocolName + "'");
        }
        return true;
      }
      else
