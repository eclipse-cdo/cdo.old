...
{
  public static final EContentsEList<?> EMPTY_CONTENTS_ELIST = new EContentsEList<Object>(null,
  ...
  public static <T> EContentsEList<T> createEContentsEList(EObject eObject)
  {
    ...
    return eStructuralFeatures == null ? EContentsEList.<T> emptyContentsEList() : new EContentsEList<T>(eObject,
        eStructuralFeatures);
  }
  ...
  public EContentsEList(EObject eObject)
  {
    ...
    this.eStructuralFeatures = ((EClassImpl.FeatureSubsetSupplier)eObject.eClass().getEAllStructuralFeatures())
        .containments();
  }
  ...
  {
    return new EContentsEList<E>(eObject, eStructuralFeatures)
    {
      @Override
      protected boolean resolve()
      {
  ...
  public static class FeatureIteratorImpl<E> implements FeatureListIterator<E>
  {
          ...
          while (featureCursor < eStructuralFeatures.length)
          {
            ...
            if (isIncluded(feature) && (!useIsSet() || eObject.eIsSet(feature)))
            {
              ...
              if (isHandlingFeatureMap || feature.isMany())
              {
                ...
                {
                  @SuppressWarnings("unchecked")
                  List<E> newValueList = (List<E>)value;
                  valueList = newValueList;
                }
                else
                {
                  @SuppressWarnings("unchecked")
                  InternalEList<E> newValueList = (InternalEList<E>)value;
                ...
                {
                  values = valueInternalEList == null ? valueList.listIterator() : valueInternalEList
                      .basicListIterator();
                }
                if (values == null ? scanNext() : scanNext(values))
                {
                  Object result = values == null ? valueInternalEList == null ? valueList.get(valueListIndex++)
                  ...
                  if (isHandlingFeatureMap)
                  {
                    ...
                    @SuppressWarnings("unchecked")
                    E newPreparedResult = (E)entry.getValue();
                    preparedResult = newPreparedResult;
                  }
                  else
                  {
                    @SuppressWarnings("unchecked")
                    E newPreparedResult = (E)result;
                    preparedResult = newPreparedResult;
          ...
          while (featureCursor > 0)
          {
            ...
            if (isIncluded(feature) && (!useIsSet() || eObject.eIsSet(feature)))
            {
              ...
              if (isHandlingFeatureMap || feature.isMany())
              {
                ...
                {
                  values = valueInternalEList == null ? valueList.listIterator(valueList.size()) : valueInternalEList
                      .basicListIterator(valueList.size());
                }
                if (values == null ? scanPrevious() : scanPrevious(values))
                {
                  Object result = values == null ? valueInternalEList == null ? valueList.get(--valueListIndex)
                      : valueInternalEList.basicGet(--valueListIndex) : values.previous();
                  if (isHandlingFeatureMap)
                  {
                    FeatureMap.Entry entry = (FeatureMap.Entry)result;
                    preparedFeature = entry.getEStructuralFeature();
    ...
    public static final ListIterator<?> EMPTY_ITERATOR = new FeatureIteratorImpl<Object>(null,
        (EStructuralFeature[])null)
    {
      @Override
      public boolean hasNext()
