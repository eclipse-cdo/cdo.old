...
public final class ToggleSLCommentAction extends Action implements IUpdate
{
  ...
  /**
   * Creates and initializes the action for the given text editor. The action configures its visual representation from
    ...
    if (!fOperationTarget.canDoOperation(operationCode))
    {
      if (shell != null)
        MessageDialog.openError(shell,
            "An error occured", "ToggleComment_error_message=An error occurred while toggling comments."); //$NON-NLS-2$
      return;
    }
  ...
  private boolean isSelectionCommented(ISelection selection)
  {
    ...
    try
    {
      ...
      ITypedRegion[] regions = TextUtilities.computePartitioning(document, fDocumentPartitioning, block.getOffset(),
  ...
  /**
   * Creates a region describing the text block (something that starts at the beginning of a line) completely containing
