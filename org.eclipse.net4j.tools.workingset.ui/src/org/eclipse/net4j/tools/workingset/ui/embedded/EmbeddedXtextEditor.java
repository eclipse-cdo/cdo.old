...
public class EmbeddedXtextEditor
{
   ...
   * @param job
   *          the synchronization job that will be scheduled/rescheduled at each modification of the editor text. It may
   *          be use to reconcile the content of the editor with something else.
   * @param style
  ...
  public EmbeddedXtextEditor(Composite control, Injector injector, int style)
  {
    fControl = control;
    fStyle = style;
    fAnnotationPreferences = org.eclipse.ui.internal.editors.text.EditorsPlugin.getDefault()
  ...
  /**
   * Creates a new EmbeddedXtextEditor. Equivalent to EmbeddedXtextEditor(control, injector, job, fileExtension,
  ...
  /**
   * Should be called only once, during initialization. Then, you should call
   * {@link #updateText(String, String, String)};
   * 
   * @param document
   * @param prefix
   * @param text
  ...
  private void menuAboutToShow(IMenuManager menu)
  {
    ...
    menu.appendToGroup(ICommonMenuConstants.GROUP_GENERATE,
        fActions.get(ITextEditorActionDefinitionIds.CONTENT_ASSIST_PROPOSALS));
  }
  ...
  private void createSourceViewer(Composite parent)
  {
    ...
    ValidationJob job = new ValidationJob(fResourceValidator, fDocument, new IValidationIssueProcessor()
    {
      ...
      public void processIssues(List<Issue> issues, IProgressMonitor monitor)
      {
        ...
        {
          annotationIssueProcessor = new AnnotationIssueProcessor(fDocument, fSourceViewer.getAnnotationModel(),
   ...
   * <p>
   * This field should not be referenced by subclasses. It is <code>protected</code> for API compatibility reasons and
   * will be made <code>private</code> soon. Use {@link #getSourceViewerDecorationSupport(ISourceViewer)} instead.
   * </p>
   */
  ...
  /**
   * Configures the decoration support for this editor's source viewer. Subclasses may override this method, but should
   * call their superclass' implementation at some point.
   * 
   * @param support
  ...
  private void configureSourceViewerDecorationSupport(SourceViewerDecorationSupport support)
  {
    ...
    Iterator<AnnotationPreference> e = Iterators.filter(fAnnotationPreferences.getAnnotationPreferences().iterator(),
  ...
  /**
   * Updates the text of this editor with the given String or the serialized form of the EObject if the semantic model
   * of the String does not contain any error and is different from the given EObject.
   * 
   * @param eObject
   * @param asString
   */
  public void update(EObject eObject, String asString)
  {
    ...
    if (eObject != null)
    {
      ...
      // fResourceSetProvider.get(null).createResource(URI.createURI("asStringResource." + fFileExtension));
      ...
      if (!asStringResource.getErrors().isEmpty() || asStringResource.getParseResult() != null
          && !asStringResource.getParseResult().getParseErrors().isEmpty())
      {
      ...
      else if (asStringEObject != null)
      {
        try
        {
          ...
          // fResourceSetProvider.get(null).createResource(URI.createURI("copyResource." + fFileExtension));
          // EmbeddedXtextResource copyResource = (EmbeddedXtextResource)
          // fResourceSetProvider.get(null).createResource(URI.createURI("copyResource." + fFileExtension));
          ...
          try
          {
            ...
            if (!equals(copyEObject, asStringEObject))
            {
              ...
              update(asString); // FIXME: should update with the serialized form of the copyEObject but throw
                                // RuntimeException!!!
            }
            else
            {
  ...
  private final class SourceViewerFocusListener implements FocusListener
  {
    ...
    public void focusLost(FocusEvent e)
    {
      ...
      {
        IContextService contextService = (IContextService)getActiveEditor().getSite().getService(IContextService.class);
    ...
    public void focusGained(FocusEvent e)
    {
      ...
      {
        fHandlerActivations.add(handlerService.activateHandler(actionHandler.getAction().getId(), actionHandler,
            fExpression));
      }
    }
